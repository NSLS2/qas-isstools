...
from isstools.widgets import (
    ...
    widget_run_diff,
    widget_sdd_manager,
    widget_beamline_status,
    widget_user_motors,
    widget_xspress3x_manager,
)
...
class XliveGui(*uic.loadUiType(ui_path)):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.widget_general_info = widget_general_info.UIGeneralInfo(
            accelerator, mono, RE, db, parent_gui=self
        )
        self.layout_general_info.addWidget(self.widget_general_info)

        if self.mono is not None:
            self.widget_trajectory_manager = (
                widget_trajectory_manager.UITrajectoryManager(mono, self.run_prep_traj)
            )
        ...
        if self.RE is not None:
            ...
            self.widget_run_diff = widget_run_diff.UIRunDiff(
                RE, db, self.pe1, self.diff_plans, parent_gui=self, mono=mono
            )
            self.layout_run_diff.addWidget(self.widget_run_diff)

            if sdd is not None:
                self.widget_sdd_manager = widget_sdd_manager.UISDDManager(
                    service_plan_funcs, sdd, RE
                )

                self.layout_xspress3_setup.addWidget(self.widget_sdd_manager)

            if xspress3x is not None:
                self.widget_xspress3x_manager = widget_xspress3x_manager.UIXSXManager(
                    service_plan_funcs, xspress3x, RE
                )
        ...
        self.widget_user_motors = widget_user_motors.UIUserMotors(
            RE,
